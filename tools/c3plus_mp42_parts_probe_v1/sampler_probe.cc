...
int main(int argc,char** argv) {
  try {
    ...
    if (object.size()<5) throw std::runtime_error("Expected at least two convex pieces followed by three supports");
    const size_t n_main=object.size()-3;
    const auto& inspector=sg.model_inspector();
    for(size_t i=0;i<object.size();++i) {
      const auto& shape=inspector.GetShape(object[i]);
      if(i<n_main && dynamic_cast<const drake::geometry::Convex*>(&shape)==nullptr)
        throw std::runtime_error("Object collision piece did not parse as Convex");
      if(i>=n_main && dynamic_cast<const drake::geometry::Sphere*>(&shape)==nullptr)
    ...
    for(size_t i=0;i<n_main;++i)groups[1].emplace_back(ee,object[i]);
    for(size_t i=n_main;i<object.size();++i)groups[2].emplace_back(object[i],ground);
    ...
    std::cout<<"[PROBE] main_convex_parts="<<n_main<<" supports=3; parsed shape roles, verified contact capacity, sampled fixed input; no simulator advanced\n";
    return 0;
