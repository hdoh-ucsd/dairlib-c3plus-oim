...
xArm6 (5-joint, joint6 welded) variant of the C3+ shelf_gap scene.
...
merged with ONLY the scene bits of `push_t_bt010_shelf_gap`:

- `parameters/scenario_params.yaml`: obstacle discs [0.73,-0.053][0.73,0.0][0.73,0.053] r=0.133 and [0.35,-0.04][0.35,0.04] r=0.064,
  `obstacle_model: examples/sampling_c3/urdf/scene_shelf_gap.sdf`
...
bazel-bin/examples/sampling_c3/franka_sim \
  --demo_name=push_t_bt010_shelf_gap_xarm6 --robot_model=xarm6 --matched_mu=true
bazel-bin/examples/sampling_c3/franka_osc_controller \
  --demo_name=push_t_bt010_shelf_gap_xarm6 --robot_model=xarm6 \
...
(0.5, 0.3) r = 0.583 and goal (0.5, -0.3) r = 0.583 — both inside.
NOTE: the big shelf (centers x = 0.73, planner discs r = 0.133) lies partly
OUTSIDE the declared 0.70 reach; that is fine (it is a static obstacle the
robot never needs to reach), but the sampler will reject EE candidates near
it via robot_radius_limits, same as the executor's 0.68 planar clamp.
