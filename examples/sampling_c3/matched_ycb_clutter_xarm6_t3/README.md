# matched_ycb_clutter_xarm6_t1

xArm6 (5-joint, joint6 welded) variant of the C3+ ycb_clutter scene (cube + sugar box + spam can + mustard bottle).
...
- `parameters/scenario_params.yaml`: the four OIM ycb_clutter obstacles as
  planner discs (cube [0.35,0,r0.0707], sugar box [0.22,0.20,r0.0996],
  spam can [0.60,-0.16,r0.0520], mustard bottle [0.15,-0.18,r0.0491]) with
  `obstacle_model: examples/sampling_c3/urdf/scene_ycb_clutter_oimframe.sdf`
  (OIM-native frame; hull-prism OBJs under urdf/ycb_hulls/).
...
bazel-bin/examples/sampling_c3/franka_sim \
  --demo_name=matched_ycb_clutter_xarm6_t1 --robot_model=xarm6 --matched_mu=true
bazel-bin/examples/sampling_c3/franka_osc_controller \
  --demo_name=matched_ycb_clutter_xarm6_t1 --robot_model=xarm6 \
